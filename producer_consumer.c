...
void *producer (void *parg)
{
  ...
  while (1) {
    ...
    //Lock the critical section with the fifo q's mutex
    ...
    while (fifo->full && *total_produced != WORK_MAX) {
      printf ("prod %d:  FULL.\n", my_tid);
      pthread_cond_wait(fifo->notFull, fifo->mutex); //Wait for room in q
    }
    ...
    if (*total_produced >= WORK_MAX) {
      pthread_mutex_unlock(fifo->mutex); //Unlock if work_max has been reached
      break;
    }
    ...
    //Typical case - Unlock mutex and broadcast not empty condition.
...
void *consumer (void *carg)
{
  ...
  while (1) {
     ...
     * si not empty.
     */
    pthread_mutex_lock(fifo->mutex); //Lock the mutex before consuming
    while (fifo->empty && *total_consumed != WORK_MAX) {
      printf ("con %d:   EMPTY.\n", my_tid);
      pthread_cond_wait(fifo->notEmpty, fifo->mutex); //Wait to lock mutex until notEmpty condition met.
    }
    ...
    if (*total_consumed >= WORK_MAX) {
      pthread_mutex_unlock(fifo->mutex); //release lock when breaking after work_max met
      break;
    }
    ...
    //Typical case - Removal is complete
    pthread_mutex_unlock(fifo->mutex);      //Unlock the mutex
    pthread_cond_broadcast(fifo->notFull);  //Broadcast notFull
